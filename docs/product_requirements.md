...
- 数据驱动不直接使用 Python 函数上的 `.mark.parametrize`，而是在 YAML-first 资产模型中表达。
- 推荐由 `ApiCase` 或 `Scenario` 声明 `data` / `datasets` / `data_source`，每一组数据必须有稳定 `data_id`，用于 nodeid、Allure 标题和历史结果定位。
- pytest collection 阶段由 AutoAPI collector 展开数据集：一个逻辑 case / scenario 可以生成多个 pytest item，例如 `case_login[normal_user]`、`case_login[wrong_password]`。
- 每个 item 保存自己的 `data_id` 和 `data_row`，执行时由 `item.runtest()` 调用 AutoAPI Executor，并把该数据行合入本轮 `override` 或 `RuntimeContext`。
- `Scenario` 数据驱动的上下文初始化继续遵守 §6.3 已锁定的三层叠加规则：`env.variables` → `dataset.variables` → 运行时 extract；不同数据轮次之间不得共享运行时 extract。
- Allure 和 JSONL history 必须保留 `case_id` / `scenario_id`、`data_id`、最终状态和首个失败 step，保证同一逻辑用例的不同数据行可以独立追踪。
- P1 未正式落地前，数据驱动效果可先通过手写多个 `case_` ID 表达，不在 P0 主链路中静默实现。
