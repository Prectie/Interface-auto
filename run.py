...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_parser.add_argument(
        "selector",
        help="执行选择器: case_xx / scn_xxx / plan_xxx / cases / scenarios / plans / all"
    )
...
class RunSelection:
    """
      ...
      collect_path: 传给 pytest 的收集路径, 可以精细到 case / scenario / plan / all
    """
    selector: str
    target: str | None
    collect_path: Path
...
def _print_run_summary(result: RunResult) -> None:
    ...
    print("  request:")
    _print_indented_json(request_payload)
    print("  response:")
    _print_indented_json(response_payload)
    print("  context:")
    _print_indented_json(problem_step.context_snapshot)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        try:
            ...
            return validate(args.data_root)
    ...
    if args.command == "run":
        data_dir = args.data_root or "Data/reading_house"
        try:
